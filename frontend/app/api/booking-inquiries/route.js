  ...
  try {
    ...
    try {
      const strapiRes = await fetch(`${strapiUrl}/api/booking-inquiries`, {
        ...
        signal: AbortSignal.timeout(3000), // 3s timeout so user never waits
      });
      if (strapiRes.ok) {
        console.log(`[HSW] Inquiry ${referenceId} synced to Strapi successfully`);
      }
    } catch (strapiErr) {
    ...
    return NextResponse.json({
      success: true,
      referenceId,
      message: 'Your inquiry has been received by HSW Global Private Advisory.',
    }, { status: 200 });
